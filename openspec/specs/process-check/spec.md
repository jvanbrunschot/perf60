...
When `/proc` cannot be listed, or no process stat file could be read in the last successful
scan, the process check SHALL be SKIPPED.
...
- **WHEN** `/proc` lists only non-numeric entries
