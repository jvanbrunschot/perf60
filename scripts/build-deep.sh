...
# Usage: scripts/build-deep.sh [--clippy] [target-triple ...]
#   --clippy  also run `cargo clippy --features deep -- -D warnings` per target (as CI does)
set -eu
cd "$(dirname "$0")/.."
clippy=0
if [ "${1:-}" = "--clippy" ]; then clippy=1; shift; fi
...
inner=$(mktemp "${TMPDIR:-/tmp}/perf60-build-deep.XXXXXX")
cat > "$inner" <<INNER
set -eu
...
[ $clippy = 1 ] && rustup component add clippy >/dev/null 2>&1
/src/scripts/install-bpf-linker.sh /usr/local/bin >/dev/null
cd /src
for t in $targets; do
  CARGO_TARGET_AARCH64_UNKNOWN_LINUX_MUSL_LINKER=rust-lld \
  CARGO_TARGET_X86_64_UNKNOWN_LINUX_MUSL_LINKER=rust-lld \
    sh -c '[ $clippy = 0 ] || cargo clippy --locked --features deep --target "\$0" -- -D warnings' "\$t"
  CARGO_TARGET_AARCH64_UNKNOWN_LINUX_MUSL_LINKER=rust-lld \
  CARGO_TARGET_X86_64_UNKNOWN_LINUX_MUSL_LINKER=rust-lld \
...
trap 'docker rm -f "$name" >/dev/null 2>&1; rm -f "$inner"' EXIT
docker cp "$inner" "$name:/build.sh"
